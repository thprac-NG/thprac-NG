...
#define SCALE (*(float*)RVA(0x20B1D0))
            ...
            virtual void OnContentUpdate() override
            {
                ...
                auto _barrier = [](PlayerBarrier* barrier, const char* label) {
                    ...
                    if (barrier->state == BARRIER_DISABLED) {
                        ...
                        // ---
                        // 1) That function is hooked to do nothing and return 0 if invincibility is
                        // enabled 1.5) I kinda wrote myself in a corner when I put all of this code
                        // *above* TH19PracHook.... 2) But none of that matters because the function
                        // does a bunch of other stuff that I don't want.
        ...
        // In the loader thread, right before the instruction that tells the main thread that
        ...
        static __declspec(noinline) void THGuiCreate()
        {
            ...
            GameGuiInit(
                ...
                GetMemContent(RVA(0x1AE3A0)) + 0x30 + 0x2B0,
                GetMemContent(RVA(0x1AE3A0)) + 0x30 + 0x10,
                0,
                -2,
                SCALE,
                0.0f
...
void TH19_v1_00a_Init()
{
    ...
    TH19::V1_00a::THInitHook::singleton().EnableAllHooks();
}
}
